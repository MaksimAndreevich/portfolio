...
import { Filter } from "../../lib/stores/interfaces/todoStore.interface";

const TodoFilterButtons = observer(() => {
  const todoStore = useStore("todoStore");

  const handleChangeFilter = (event: React.MouseEvent<HTMLElement>, newAlignment: Filter) => {
    newAlignment && todoStore.changeFilter(newAlignment);
  };
  return (
    <ToggleButtonGroup color="info" value={todoStore.filter} exclusive aria-label="toggle todo filter" fullWidth onChange={handleChangeFilter}>
